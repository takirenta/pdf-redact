...
情報公開で起こりがちな事故（黒塗り漏れ・復元・元ファイル破壊）を、
**構造的に防ぐ**ように設計されています。

| 防ぐ事故 | 設計上の対策 |
|---|---|
| 黒塗りしたのにコピペで復元される | 見た目に重ねるだけではなく、保存時に該当領域のテキスト・画像を PDF から**物理削除**。未参照データの残骸もファイルから除去（`garbage=4`） |
| 黒塗り漏れで情報が残る | 領域と**一部でも交差**するテキスト行・画像は**全体を削除**（漏えい防止のため安全側に倒す）。画像の一部だけ黒塗りしても画像全体が消える |
| 元の PDF を上書き・破壊する | 保存は常に**別名保存**。元ファイルはバイト単位で不変（テストで検証済み） |
| 誤操作で保存してしまう | 保存前に確認ダイアログで確認（保存実行・上書きの両方で確認） |
| パスワード付き PDF で事故 | パスワード付き PDF は**開かずに**日本語エラーで拒否 |
| 壊れた PDF で不正な処理 | 破損・非 PDF ファイルは日本語エラーで拒否 |
| ドラッグがページからはみ出す | 黒塗り矩形を自動的にページ内へ切り詰め |

- **完全オフライン** — 通信コードは一切ありません。文書データが外部に出る経路は存在しません（依存は PyMuPDF のみ）
- **検証済み** — `test_redact.py` の 11 件のテスト（テキスト削除・画像削除・元ファイル不変・黒の焼き込み・エラー処理）をすべて PASS。さらに実際の住所・電話番号を黒塗り → 保存後の PDF から抽出して消えていることを確認済み
...
（例では、実際に公開されている審議会資料の出席者氏名を黒塗りしています）。
